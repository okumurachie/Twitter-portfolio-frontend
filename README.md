...
Nuxt 4 を使用してUIを構築し、LaravelのバックエンドAPIと連携しています。
...
2. Vueコンポーネントの分割
    - 再利用性と保守性を意識し、投稿フォームを独立したコンポーネントとして切り出しました。
    - 投稿機能は複数画面で利用されることを想定し、別画面でも再利用可能な構成にしています。
    - UIの見た目だけでなく、将来的な機能追加や修正を考慮した設計を意識しました。
...
- フロントはNuxtのSPAで構築し、認証はFirebase Authenticationを利用しています。</br>
  ログイン後に取得したIDトークンをBearerトークンとしてLaravel APIに送信します。</br>
  Laravel側ではミドルウェアでFirebaseのトークン検証を行い、認証済みユーザーのみDB操作を許可する構成にしています。

### 設計意図

- フロントとバックエンドを分離することで、それぞれの責務を明確化
- Firebase Authenticationを利用することで、安全なトークン認証を実現
- SPA構成により、快適なユーザー体験を提供
